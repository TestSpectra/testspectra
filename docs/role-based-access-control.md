## Multi-Tenancy & Project Membership

### Project-Agnostic Roles (Global)
Roles in TestSpectra are **Global**. A user's role (e.g., `admin`, `qa_lead`, `qa_engineer`) is defined at the system level (`public.users` table) and does not change between projects.

### Project Membership
Users must be members of a project to access its data. Membership is stored in `public.project_members`.

### Project Assignment Logic
| Role | Assignment Capability |
|------|----------------------|
| **Admin** | Can assign **any user** to any project. |
| **QA Lead** | Can assign users with the **QA Engineer** role to any project. |
| **Others** | Cannot manage project membership. |

### Access Control in Code
- **Global Permissions**: Checked via `check_permissions(db, user_id, &[permissions])`.
- **Data Isolation**: Each project has its own PostgreSQL schema. Handers use a per-request `search_path` to ensure data isolation.
- **Membership Check**: UI and API routes filter data based on whether the user is registered in `public.project_members` for that specific project.
