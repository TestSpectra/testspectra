...
export function MobileInspector() {
  ...
  const inspectorUrl = sessionId
    ? `http://127.0.0.1:4723/inspector?state=${JSON.stringify({ attachSessId: sessionId })}&autoStart=1`
  ...
  useEffect(() => {
    ...
    const checkInspector = async () => {
      try {
        const response = await fetch("http://127.0.0.1:4723/inspector", {
          mode: "no-cors",
        });
