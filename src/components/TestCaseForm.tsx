...
export function TestCaseForm({ testCase, onSave, onCancel }: TestCaseFormProps) {
  const [formData, setFormData] = useState({
    ...
    automationStatus: testCase?.automation === 'Automated' ? 'automated' : 'manual',
    filePath: testCase?.filePath || '',
    caseType: testCase?.caseType || 'Positive',
  });

  const [actions, setActions] = useState<TestAction[]>(testCase?.actions || [
  ...
  const handleGenerateScript = () => {
    alert('Script skeleton akan dibuat di working directory:\n\n/tests/' + formData.suite.toLowerCase().replace(/\s+/g, '-') + '/' + formData.id.toLowerCase() + '.test.ts');
  };
    ...
    <div className="p-8 bg-slate-950">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="mb-2">{isEditing ? 'Edit Test Case' : 'Create New Test Case'}</h1>
            <p className="text-slate-400">
              {isEditing ? `Mengedit ${testCase.id}` : 'Tambahkan test case baru dengan action-based steps'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={onCancel} className="border-slate-600 text-slate-100 hover:bg-slate-800 hover:border-slate-500">
        ...
        <div className="col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm text-slate-400 mb-2">Test Case ID</label>
                <input
                  type="text"
                  value={formData.id}
              ...
              <div>
                <label className="block text-sm text-slate-400 mb-2">Test Suite/Module</label>
                <input
                  type="text"
            ...
            <div className="mb-4">
              <label className="block text-sm text-slate-400 mb-2">Prioritas</label>
              <select
            ...
            <div className="mb-4">
              <label className="block text-sm text-slate-400 mb-2">Tipe Test Case</label>
              <select
              ...
              <p className="text-xs text-slate-500 mt-2">
                Positive: Skenario normal/happy path | Negative: Skenario error handling | Edge: Corner case/boundary conditions
              </p>
            </div>

            <div>
              <label className="block text-sm text-slate-400 mb-2">Expected Outcome / Final Assertion *</label>
              <textarea
              ...
              />
              <p className="text-xs text-slate-500 mt-2">This field captures the pass condition for this test case</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-slate-400">Define test actions in sequence</p>
              </div>
              <Button 
            ...
            <div className="space-y-4">
              {actions.map((action, index) => (
                <div key={action.id} className="flex items-start gap-3 bg-slate-800/50 p-4 rounded-lg">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant="outline" className={`${getActionColor(action.type)} border shrink-0`}>
                        {action.type}
                      </Badge>
                    </div>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg cursor-pointer hover:bg-slate-800 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Dijalankan oleh QA manual</p>
                </div>
              </label>
              <label className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg cursor-pointer hover:bg-slate-800 transition-colors">
                <input
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Dijalankan oleh automation script</p>
                </div>
              </label>
            </div>
          </div>
          ...
          {formData.automationStatus === 'automated' && (
            <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
              ...
              <div className="mb-4">
                ...
                />
                <p className="text-xs text-slate-500 mt-2">Path diisi otomatis dari sinkronisasi kode</p>
              </div>
              ...
              <p className="text-xs text-slate-500 mt-3">
                Akan membuat file .test.ts kosong di working directory dengan struktur dasar berdasarkan actions yang didefinisikan
              </p>
            </div>
          )}
